...
namespace Linq2d
{
	public static partial class Array2d
	{
        ...
        public static IArrayTransform2<T1, T2, R1, R2> SelectMany<T1, T2, R1, R2>(this ArraySource<T1> source, Func<object, T2[,]> secondSelector, Expression<Func<Cell<T1>, Cell<T2>, (R1, R2)>> resultSelector) 
            => new ArrayQuery2<T1, T2, R1, R2>(source, secondSelector(default).Wrap(), resultSelector);

        public static IArrayTransform2<T1, T2, R1, R2> SelectMany<T1, T2, R1, R2>(this ArraySource<T1> source, Func<object, ArraySource<T2>> secondSelector, Expression<Func<Cell<T1>, Cell<T2>, (R1, R2)>> resultSelector) 
            => new ArrayQuery2<T1, T2, R1, R2>(source, secondSelector(default), resultSelector);

		// final select of 3 results
        public static IArrayTransform3<T1, T2, R1, R2, R3> SelectMany<T1, T2, R1, R2, R3>(this ArraySource<T1> source, Func<object, T2[,]> secondSelector, Expression<Func<Cell<T1>, Cell<T2>, (R1, R2, R3)>> resultSelector) 
            => new ArrayQuery3<T1, T2, R1, R2, R3>(source, secondSelector(default).Wrap(), resultSelector);
